"""
NCAA estimator sweep: XGB+CAT+RF (no GBM) from 100 to 175 in steps of 5.
Run from sports-predictor-api root:
    python3 ncaa_sweep_no_gbm.py
"""
...
print("=" * 70)
print("NCAA ESTIMATOR SWEEP: XGB+CAT+RF (no GBM), 100 → 175 (step 5)")
print("=" * 70)
...
print(f"{'Est':>5} | {'MAE':>8} | {'RF':>7} {'XGB':>7} {'CAT':>7} | {'Time':>6}")
print("-" * 60)
...
for n_est in range(100, 180, 5):
    ...
    w_rf = weights[0]
    w_xgb = weights[1] if HAS_XGB else 0
    w_cat = weights[2] if HAS_CAT else (weights[1] if not HAS_XGB else 0)

    print(f"{n_est:>5} | {mae:>8.3f} | {w_rf:>+7.3f} {w_xgb:>+7.3f} {w_cat:>+7.3f} | {elapsed:>5.1f}s")

    results.append({
        "n_est": n_est, "mae": mae,
        "w_rf": float(w_rf), "w_xgb": float(w_xgb), "w_cat": float(w_cat),
        "duration": elapsed,
    })
...
print(f"  Weights: RF={best['w_rf']:+.4f}, XGB={best['w_xgb']:+.4f}, CAT={best['w_cat']:+.4f}")
