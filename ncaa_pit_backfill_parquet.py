...
def main():
    ...
    parser.add_argument("--push-season", type=int, default=None,
                        help="Only push updates for this season (computes all for Elo accuracy)")
    ...
    # Filter by season if --push-season is set
    if args.push_season:
        gid_to_season = {g.get("game_id"): int(g.get("season", 0)) for g in all_games}
        filtered = [(gid, patch) for gid, patch in updates if gid_to_season.get(gid) == args.push_season]
        print(f"\n  Phase 3: Pushing {len(filtered)}/{len(updates)} updates (season {args.push_season} only)...")
        push_list = filtered
    else:
    ...
    success = 0
    for i, (gid, patch) in enumerate(push_list):
        if sb_patch("ncaa_historical", "game_id", gid, patch): success += 1
        if (i + 1) % 200 == 0: print(f"    {i+1}/{len(push_list)} ({success} ok)")
        time.sleep(0.02)

    print(f"\n{'='*70}")
    print(f"  v3 BACKFILL COMPLETE — 107 FEATURES")
    print(f"{'='*70}")
    print(f"  Games: {len(all_games)} | Computed: {computed} | Updated: {success}/{len(push_list)}")
    if args.push_season:
        print(f"  Push filter: season {args.push_season} only")
