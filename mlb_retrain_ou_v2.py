#!/usr/bin/env python3
"""
mlb_retrain_ou_v2.py — MLB Over/Under v2: Triple Agreement
═══════════════════════════════════════════════════════════════
Architecture SEARCH — no assumptions from other sports.
Tests linear-only, tree-only, and mixed ensembles for each component.
Walk-forward picks the best per-component AND cross-config combo.

Components:
  Residual models  → predict where Vegas O/U is wrong
  Classifiers      → predict P(under)
  ATS score models → predict home/away runs separately

Usage:
    python3 mlb_retrain_ou_v2.py                # Evaluate all configs
    python3 mlb_retrain_ou_v2.py --upload       # Train best + upload
    python3 mlb_retrain_ou_v2.py --refresh      # Re-pull data first
"""
...
# Residual: detect Vegas O/U mispricing. Environment + bullpen + ump.
RES_FEATURE_COLS = [
...
def main():
    ...
    bundle = {
        ...
        "model_type": "mlb_ou_v2_triple_agreement",
        ...
        "trained_at": time.strftime("%Y-%m-%d %H:%M:%S"), "_v2_triple": True,
    }
