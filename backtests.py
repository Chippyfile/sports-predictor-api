...
# ═══════════════════════════════════════════════════════════════
# NCAA CONFIDENCE & CALIBRATION DIAGNOSTICS
# ═══════════════════════════════════════════════════════════════

def ncaa_confidence_calibration():
    """NCAA confidence/calibration analysis — tiers based on win probability margin."""
    ...
    if not rows or len(rows) < 100:
        return jsonify({"error": "Need 100+ graded NCAA games", "n": len(rows) if rows else 0})
...
# ═══════════════════════════════════════════════════════════════
# MLB CONFIDENCE & CALIBRATION DIAGNOSTICS
# ═══════════════════════════════════════════════════════════════

def mlb_confidence_calibration():
    """MLB confidence/calibration analysis — tiers based on win probability margin."""
    rows = sb_get("mlb_predictions",
                  "result_entered=eq.true&ml_correct=not.is.null"
                  "&select=ml_correct,rl_correct,ou_correct,win_pct_home,confidence,market_ou_total,market_spread_home,game_type"
                  "&order=game_date.asc&limit=5000")
    if not rows or len(rows) < 50:
        return jsonify({"error": "Need 50+ graded MLB games", "n": len(rows) if rows else 0})

    df = pd.DataFrame(rows)
    # Filter to regular season only (exclude spring training)
    if "game_type" in df.columns:
        df = df[df["game_type"].isin(["R", "F", "D", "L", "W"])]  # Regular + postseason
    df["win_pct_home"] = pd.to_numeric(df["win_pct_home"], errors="coerce").fillna(0.5)
    df["ml_correct"] = df["ml_correct"].astype(bool)
    df["conf_margin"] = (df["win_pct_home"] - 0.5).abs()

    if len(df) < 50:
        return jsonify({"error": "Need 50+ regular season graded MLB games", "n": len(df)})

    # MLB: tightest margins, highest variance → lowest thresholds
    MLB_HIGH = 0.10   # ≥60% win prob → HIGH
    MLB_MED  = 0.04   # ≥54% win prob → MEDIUM
    tier_results = {}
    for tier, lo, hi in [("LOW", 0, MLB_MED), ("MEDIUM", MLB_MED, MLB_HIGH), ("HIGH", MLB_HIGH, 0.5)]:
        subset = df[(df["conf_margin"] >= lo) & (df["conf_margin"] < hi)] if tier != "HIGH" else df[df["conf_margin"] >= lo]
        if len(subset) > 0:
            ats_ou = _ats_ou_stats(subset)
            tier_results[tier] = {
    ...
    decile_results = []
    thresholds = [0.0, 0.02, 0.04, 0.06, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30]
    for j in range(len(thresholds) - 1):
        lo, hi = thresholds[j], thresholds[j + 1]
        subset = df[(df["conf_margin"] >= lo) & (df["conf_margin"] < hi)]
        if len(subset) >= 5:
    ...
    cumulative = []
    for threshold in [0.02, 0.04, 0.06, 0.08, 0.10, 0.15, 0.20, 0.25]:
        subset = df[df["conf_margin"] >= threshold]
        if len(subset) >= 5:
            cumulative.append({
                "min_margin": threshold,
    ...
    for row in cumulative:
        if row["accuracy"] >= 0.55 and suggested_medium is None:
            suggested_medium = row["min_margin"]
        if row["accuracy"] >= 0.60 and suggested_high is None:
            suggested_high = row["min_margin"]
    ...
    return jsonify({
        ...
        "by_tier": tier_results,
        "data_quality_tiers": dq_results,
        "tier_thresholds": {"LOW": f"<{MLB_MED}", "MEDIUM": f"{MLB_MED}-{MLB_HIGH}", "HIGH": f">={MLB_HIGH}"},
        "by_margin_decile": decile_results,
        "cumulative_threshold": cumulative,
        "ats_cumulative_threshold": ats_cumulative,
        "ou_cumulative_threshold": ou_cumulative,
        "suggested_thresholds": {
            "MEDIUM_min_margin": suggested_medium,
            "HIGH_min_margin": suggested_high,
            "note": "MLB: 55% / 60% thresholds (highest variance sport)"
        },
    })
